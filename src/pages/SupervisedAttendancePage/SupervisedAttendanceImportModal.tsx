...
import {
    FILE_UPLOAD,
    EXPORT_ATTENDANCE_RAW_TEMPLATE,
    PREPARE_IMPORT_ATTENDANCE,
    PREVIEW_IMPORT_ATTENDANCE,
    CONFIRM_IMPORT_ATTENDANCE,
    CLEAR_IMPORT_ATTENDANCE
} from '../../config/api-routes';
import { useAuthStore } from '../../stores/auth-store';
import styles from './SupervisedAttendanceImportModal.module.css';

interface SupervisedAttendanceImportModalProps {
    open: boolean;
    onClose: () => void;
    onSuccess: () => void;
}

export default function SupervisedAttendanceImportModal({ open, onClose, onSuccess }: SupervisedAttendanceImportModalProps) {
    ...
    const downloadTemplate = async () => {
        try {
            setExporting(true);
            const res = await apiClient.post(EXPORT_ATTENDANCE_RAW_TEMPLATE, {
                userid: userId || '',
                domain: domain || 'dev'
            });
            const data = res.data?.data || res.data;
            if ((res.data?.status === 201 || res.data?.statuscode === 200 || res.data?.statuscode === 300) && data?.base64String) {
                const link = document.createElement('a');
                link.href = `data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,${data.base64String}`;
                link.download = data.fileName || 'Attendance_Template.xlsx';
            ...
            return new Promise((resolve, reject) => {
                ...
                reader.onload = async () => {
                    ...
                    try {
                        ...
                        // 2. Prepare Import
                        const prepareRes = await apiClient.post(`${PREPARE_IMPORT_ATTENDANCE}?fileName=${uploadedFileName}`, {
                            userid: user?.userid || '',
                            domain: domain || 'dev'
                        });
                        ...
                        const previewRes = await apiClient.post(`${PREVIEW_IMPORT_ATTENDANCE}/${batchid}`, previewPayload);
                        const previewResult = previewRes.data?.data || previewRes.data;
    ...
    const confirmMutation = useMutation({
        mutationFn: async () => {
            ...
            const res = await apiClient.post(`${CONFIRM_IMPORT_ATTENDANCE}/${previewData.batchid}`, {
                userid: user?.userid || '',
                domain: domain || 'dev'
            });
            const data = res.data;
    ...
    const handleClose = async () => {
        if (previewData?.batchid && !confirmMutation.isSuccess) {
            try {
                await mainClient.post(`${CLEAR_IMPORT_ATTENDANCE}/${previewData.batchid}`, {
                    userid: user?.userid || '',
                    domain: domain || 'dev'
                });
            } catch (err) {
            ...
            <div className={styles['import-modal']}>
                <div className={styles['import-modal__steps']}>
                    <div className={styles['import-modal__step']}>
                        ...
                        >
                            <Download size={14} />
                            {exporting ? 'Preparing template...' : 'Download Attendance_Template.xlsx'}
                        </button>
                    </div>
